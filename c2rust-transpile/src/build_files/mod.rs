...
pub fn emit_build_files<'lcmd>(
    ...
    if let Some(ccfg) = crate_cfg {
        ...
        if tcfg.no_split_library {
            tcfg.binaries.iter().flat_map(|binary| {
                emit_lib_rs(
                    tcfg,
                    &reg,
                    build_dir,
...
fn emit_lib_rs(
    ...
    if current_binary.is_some() {
        for elem in modules
            .iter()
            .find(|m| {
                tcfg.maybe_binary_name_from_path(m).as_deref() == current_binary
            })
            .unwrap()
    ...
    let modules = convert_module_list(tcfg, build_dir, modules, ModuleSubset::Libraries, &current_binary);
